...
public class BookingRecordResponseTest extends AbstractModelBeanTest<BookingRecordResponse>{
    ...
    private Set<PassengerResponse> passengersResponseOtherSet = new HashSet<PassengerResponse>();
    ...
    public void initEntities(){
        ...
        passengersResponseOtherSet.add(passengerResponseOther);

        entityA1 = bookingRecordResponse.cloneBuilder().withPassengersResponse(passengersResponseSet).build();
        entityA2 = bookingRecordResponse.cloneBuilder().withPassengersResponse(passengersResponseSet).build();
        entityB = bookingRecordResponseOther.cloneBuilder().withPassengersResponse(passengersResponseOtherSet).build();
